...
export default {
  ...
  methods: {
    //造数据测试
    test_get_data() {
      this.tableList.baseInfo.line = 123;
      this.tableList.baseInfo.onlineNum = 123;
      this.tableList.baseInfo.realOnlineNum = 123;
      ...
      for (let i = 0; i < 6; i++) {
        if (res.Plan[i]) {
          let obj = {
            ...
            customer: "客户名称新科技", //客户名称
            prno: "拉线车间新科技产品,锻造工艺3333巴博萨巴博萨", //产品名称
            //ptno:res.Plan[i].Order_No != undefined ? res.Plan[i].Order_No : "", //产品编号
            plantNo: "123", //计划产量
            realNo: "123", //实际产量
            reachRate: 12, //达成率
            notGoodNum: "123123", //不良数
            notGood: 12123 //不良率
          };
          planTotal += res.Plan[i].Sch_Qty;
          realTotal += res.Plan[i].Output_Qty;
          list.push(obj);
        } else {
          let obj = {
            line: "", //拉线
        ...
        for (let i = 0; i < 6; i++) {
          if (res.Plan[i]) {
            let obj = {
              ...
              //ptno:res.Plan[i].Order_No != undefined ? res.Plan[i].Order_No : "", //产品编号
              plantNo:
                res.Plan[i].Sch_Qty != undefined ? res.Plan[i].Sch_Qty * 1 : 0, //计划产量
              realNo:
                res.Plan[i].Output_Qty != undefined
                  ? res.Plan[i].Output_Qty * 1
                  : 0, //实际产量
              reachRate:
              ...
              notGoodNum:
                res.Plan[i].Reach_Rate != undefined
                  ? res.Plan[i].Reach_Rate
                  : "", //不良数
              notGood:
                res.Plan[i].Bad_Rate != undefined
                  ? res.Plan[i].Bad_Rate.substring(
                      0,
                      res.Plan[i].Bad_Rate.length - 1
                    )
                  : "" //不良率
            };
            planTotal += parseInt(res.Plan[i].Sch_Qty);
            realTotal += parseInt(res.Plan[i].Output_Qty);
            list.push(obj);
          } else {
            let obj = {
              line: "", //拉线
