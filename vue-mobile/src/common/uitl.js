...
//安卓环境下提供接口地址给安卓端
export const androidGetUrl = function () {
  try {
    let url = 'http://' + window.location.host + '/esop/';
    window.android.getUrl(url);
  } catch (error) {
    console.log("测试环境")
  }
}
//安卓环境下获取扫描返回信息
export const androidGetBarCodeWait = function () {
  try {
    return window.android.getBarCode();
  } catch (error) {
    console.log("测试环境")
  }
}
//安卓环境下获取扫描返回信息得到了
export const androidGetBarCodeEnd = function () {
  try {
    window.android.setBarCode("12345");
  } catch (error) {
    console.log("测试环境")
  }
}
...
//安卓环境监听获取扫描信息方法
export const androidSetBarCode = function (callback) {
  window.setBarCode = function (res) {
    callback(res);
  }
}
