...
export default {
  ...
  methods: {
    ...
    mac_init() {
      ...
      this.macInfo.equipNo = mac.equipNo;
      this.macInfo.empNo = mac.empNo;
      this.macInfo.plant = mac.plant;
      this.macInfo.line = mac.line;
      this.getAdnList();
    },
    //获取列表信息,
    getAdnList() {
      ...
      this.$get_noToken({
        url: this.$api_baseurl(this.$url.abnDailywork_list),
    ...
    typeSelect_init(type) {
      this.$get_noToken({
        url: this.$api_baseurl(this.$url.abnDailywork_type)
      }).then(res => {
        if (res.code == 1) {
          this.typeSelect = [];
    ...
    recTypeChange(recType) {
      if (this.isDailyStart && recType) {
        this.$get_noToken({
          url: this.$api_baseurl(this.$url.abnDailywork_forOne),
          params: {
            recType: recType,
    ...
    dailyStart() {
      ...
      this.$post_noToken({
        url: this.$api_baseurl(this.$url.abnDailywork_add),
        data
      }).then(res => {
        if (res.code == 1) {
          this.getAdnList();
